...
int main () {
  ...
  for (istep=1;istep<nstep;istep++) {
	...
	Lapl = 0.0; /* Initialise finite-difference Laplacian */
        grad = 0.0; /* Initialise gradient */
	
        /* compute d2u/dx2, accounting for special cases near boundaries */
        if (ix==0) {
	  Lapl += -2.0*u[ix][iy]/pow(dx,2.0) + u[ix+1][iy]/pow(dx,2.0) + u[Nx-1][iy]/pow(dx,2.0);  /* left */
          grad += (u[ix+1][iy] - u[Nx-1][iy])/dx/2.0;
        } else if (ix==Nx-1) {
	  Lapl += -2.0*u[ix][iy]/pow(dx,2.0) + u[0][iy]/pow(dx,2.0) + u[ix-1][iy]/pow(dx,2.0);     /* right */
          grad += (u[0][iy] - u[ix-1][iy])/dx/2.0;
        } else {
	  Lapl += -2.0*u[ix][iy]/pow(dx,2.0) + u[ix+1][iy]/pow(dx,2.0) + u[ix-1][iy]/pow(dx,2.0);
          grad += (u[ix+1][iy] - u[ix-1][iy])/dx/2.0;
        }
        
        /* compute d2u/dy2, accounting for special cases near boundaries */
        if (iy==0) {
	  Lapl += -2.0*u[ix][iy]/pow(dy,2.0) + u[ix][iy+1]/pow(dy,2.0) + u[ix][Ny-1]/pow(dy,2.0);  /* bottom */
          grad += (u[ix][iy+1] - u[ix][Ny-1])/dy/2.0;
        } else if (iy==Ny-1) {
	  Lapl += -2.0*u[ix][iy]/pow(dy,2.0) + u[ix][0]/pow(dy,2.0) + u[ix][iy-1]/pow(dy,2.0);     /* top */ 
          grad += (u[ix][0] - u[ix][iy-1])/dy/2.0;
        } else {
	  Lapl += -2.0*u[ix][iy]/pow(dy,2.0) + u[ix][iy+1]/pow(dy,2.0) + u[ix][iy-1]/pow(dy,2.0);  
