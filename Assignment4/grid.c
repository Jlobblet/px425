...
void grid_initialise_local(int Ngrid, int p, int my_rank, int* my_rank_coords) {
    ...
    if (grid_spin == NULL) {
        printf("Error allocating grid_spin pointers\n");
        exit(EXIT_FAILURE);
    }
    for (iy = 0; iy < grid_domain_size; iy++) {
        grid_spin[iy] = (int*) malloc(grid_domain_size * sizeof(int));
        if (grid_spin[iy] == NULL) {
            printf("Error allocating grid_spin array\n");
            exit(EXIT_FAILURE);
        }
    }
    ...
    if (grid_field == NULL) {
        printf("Error allocating grid_field pointers\n");
        exit(EXIT_FAILURE);
    }
    for (iy = 0; iy < grid_domain_size; iy++) {
        grid_field[iy] = (double*) malloc(grid_domain_size * sizeof(double));
        if (grid_field[iy] == NULL) {
            printf("Error allocating grid_field array\n");
            exit(EXIT_FAILURE);
        }
    }
    ...
    if (grid_halo == NULL) {
        printf("Error allocating grid_halo pointers\n");
        exit(EXIT_FAILURE);
    }
    for (idir = 0; idir < 4; idir++) {
        grid_halo[idir] = (int*) malloc(grid_domain_size * sizeof(int));
        if (grid_halo[idir] == NULL) {
            printf("Error allocating grid_halo array\n");
            exit(EXIT_FAILURE);
        }
    }
    ...
    if (grid_neighbours == NULL) {
        printf("Error allocating grid_neighbour pointers\n");
        exit(EXIT_FAILURE);
    }
    for (iy = 0; iy < grid_domain_size; iy++) {
        grid_neighbours[iy] = (neighbour_ptr_t*) malloc(grid_domain_size * sizeof(neighbour_ptr_t));
        if (grid_neighbours[iy] == NULL) {
            printf("Error allocating grid_neighbour array\n");
            exit(EXIT_FAILURE);
        }
    }
...
void grid_initialise_global(int Ngrid, int my_rank, int p) {
    ...
    if (my_rank == 0 && p > 1) {
        ...
        if (global_grid_spin == NULL) {
            printf("Error allocating global_grid_spin pointers\n");
            exit(EXIT_FAILURE);
        }
        for (iy = 0; iy < Ngrid; iy++) {
            global_grid_spin[iy] = (int*) malloc(Ngrid * sizeof(int));
            if (global_grid_spin[iy] == NULL) {
                printf("Error allocating global_grid_spin array\n");
                exit(EXIT_FAILURE);
            }
